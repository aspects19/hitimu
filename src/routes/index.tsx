...
function Index() {
  ...
  return (
    <div className='flex flex-col items-center'>
      ...
      <p className="text-3xl font-extrabold text-center mt-15 mb-3"> Get free study materials </p>
      <p className='text-xl font-normal'> Lecture notes, past papers, and more! </p>
      <span className=" mt-4">All in one place</span><br/>
      
      <SearchBar/>
      <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 grid-col-1  lg:grid-cols-3   mt-10'>
      ...
      <div className='flex flex-col mt-12 items-center'>
        ...
        <a   href='/upload'>
          <button className='btn mt-5 btn-primary'>Upload a study material</button>
        </a>
      </div>

      <div className="mt-20 w-full max-w-6xl px-4">
          <h2 className="text-lg font-semibold text-center mb-4 text-primary">Available material</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
            {categories.map(subject => (
              <div
              key={subject.name}
              className="card bg-base-100 shadow-md p-4 flex flex-col items-center justify-center hover:bg-primary hover:text-white transition"
              >
              {subject.icon}
              <span className="text-lg font-medium mt-2">{subject.name}</span>
              </div>
            ))}
            </div>
        </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
      <div className="space-y-4">
        ...
        <p className="text-sm text-gray-400"> Hi, I'm <span className='font-bold text-gray-200'>Jeffarson Amenya</span>. A telecommunication student who believes information should be available freely and in the open domain.
        </p>
        <a href="https://github.com/aspects19" className="text-gray-400 hover:text-white transition-colors">
