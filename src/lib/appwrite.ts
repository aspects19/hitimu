...
export const uploadFile = async (
  ...
  const response = await storage.createFile(config.documentBucketId, ID.unique(), file, [
    'read("any")',
  ]);

  const fileUrl = storage.getFileView(config.documentBucketId, response.$id);
...
export const getFilePreview = async (fileId: string): Promise<string> => {
  const filePreview = storage.getFilePreview(config.documentBucketId, fileId, 200, 200);
  return filePreview;
};
...
export const getUserDocs = async (
  ...
  const posts = await database.listDocuments(config.databaseId, config.documentCollectionId, [
    Query.equal('uploader', userId),
    Query.limit(limit),
  ]);
...
export const getSearchDocs = async (
  query: string,
  limit: number = 10
): Promise<Models.Document[]> => {
